...
(defn refresh-missions []
  ...
  (swap! app-state assoc :loading true)
  (go
    (let [response (<! (http/get (str config/api-base-url "/missions")))]
      (if (= (:status response) 200)
        (do
          (swap! app-state assoc :missions (:missions (:body response)))
          (swap! app-state assoc :loading false)
...
(defn mission-card [mission]
  "Display a single mission card with challenge analysis"
...
(defn create-mission-dialog []
  "Modal dialog for creating new missions"
...
(defn missions-page []
  "Main missions page with grid layout and pagination"
  [:div.missions-page
   [:div.page-header
    ...
    [:div.admin-section
     ...
     [:div.admin-cards
      ...
      [:div.admin-card
       ...
       [:button.btn.btn-secondary {:on-click #(.click (.querySelector js/document "input[type=file]"))}
        "📤 Choose File"]]]]
     ...
     [:div.stats-grid
      ...
      [:div.stat-card
       ...
       [:div.stat-value (if (seq (:missions @app-state))
                          (let [difficulties (filter number? (map :difficulty (:missions @app-state)))]
                            (if (seq difficulties)
                              (.toFixed (/ (reduce + difficulties) (count difficulties)) 1)
                              "N/A"))
                          "N/A")]]]]
